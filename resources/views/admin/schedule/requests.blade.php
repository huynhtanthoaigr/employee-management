    ...
    <div class="container-fluid" style="padding-top:80px; padding-bottom:50px;">
        ...
        <form action="{{ route('admin.schedule.requests.save') }}" method="POST">
                        ...
                        @foreach($employees as $emp)
                            <tr>
                                ...
                                @foreach($week as $day)
                                    ...
                                    @endphp
                                    <td>
                                        <select name="requests[{{ $emp->id }}][{{ $day }}]"
                                            class="form-select form-select-sm shadow-sm">
                                            <option value="present" {{ $status == 'present' ? 'selected' : '' }}>✅ Đi làm</option>
                                            <option value="morning_off" {{ $status == 'morning_off' ? 'selected' : '' }}>🌅 Nghỉ sáng
                                            </option>
                                            <option value="afternoon_off" {{ $status == 'afternoon_off' ? 'selected' : '' }}>🌆 Nghỉ chiều
                                            </option>
                                            <option value="full_day_off" {{ $status == 'full_day_off' ? 'selected' : '' }}>🛌 Nghỉ cả ngày
                                            </option>
                                        </select>
                                    </td>
                                @endforeach
            ...
            <div class="d-flex gap-2 mt-3">
                ...
                <button type="button" class="btn btn-warning btn-lg rounded-pill shadow-sm" data-bs-toggle="modal"
                    data-bs-target="#confirmGenerateModal">⚡ Tạo lịch tuần</button>
            </div>
        </form>
